...
export default function Header({
  ...
  if (locale === 'vn') {
    links = require("@/lib/data-vn").links;
  } else  {
    links = require("@/lib/data").links;
  }
        ...
        <ul className="flex w-[22rem] flex-wrap items-center justify-center gap-y-1 text-[0.9rem] font-medium text-gray-500 sm:w-[initial] sm:flex-nowrap sm:gap-5">
          {links.map((link: Section) => (
            ...
            >
              <Link
                className={clsx(
                  "flex w-full items-center justify-center px-2.5 py-3 hover:text-gray-950 transition dark:text-gray-500 dark:hover:text-gray-300",
                  {
